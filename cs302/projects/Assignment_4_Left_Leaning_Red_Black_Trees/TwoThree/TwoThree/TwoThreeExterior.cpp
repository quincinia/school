...
//		assert(l1 == pNode->Low1);
//		assert(l2 == pNode->Low2);
//		assert(l3 == pNode->Low3);
	}
}
...
void TwoThreeExterior::SanityCheckRec(TwoThreeExterior_t *pNode)
{
	...
	if (pNode->IsLeaf) {
		assert(pNode->Low1 == pNode->Ref.Key);
		assert(0 == pNode->Low2);
		assert(0 == pNode->Low3);
		assert(NULL == pNode->pChild1);
		assert(NULL == pNode->pChild2);
		assert(NULL == pNode->pChild3);
	}
	else {
		assert(NULL != pNode->pChild1);
		assert(NULL != pNode->pChild2);

		assert(pNode->Low1 == pNode->pChild1->Low1);
		assert(pNode->Low2 == pNode->pChild2->Low1);

		assert(pNode->Low1 < pNode->Low2);

		if (NULL != pNode->pChild3) {
			assert(pNode->Low3 == pNode->pChild3->Low1);
			assert(pNode->Low2 < pNode->Low3);
		}

		if (pNode->pChild1->IsLeaf) {
			assert(pNode->pChild2->IsLeaf);

			if (NULL != pNode->pChild3) {
				assert(pNode->pChild3->IsLeaf);
			}
		}
