...
//	RedBlackBasic       twoThree;
	...
	assert(ArraySize(ary) == twoThree.KeyCount());
	...
	for (U32 i = 0; i < ArraySize(ary) / 2; ++i) {
		assert(NULL != twoThree.LookUp(ary[i]));
	}
	...
	for (U32 i = 0; i < lo; ++i) {
		assert(NULL != twoThree.LookUp(i+1));
	}
